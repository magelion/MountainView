...
vec3 normalGrid(){
    ...
    vec4 xf = texture2D(nmap, coord.xy+vec2(1.0/512, 0.0));
    vec4 xb = texture2D(nmap, coord.xy-vec2(1.0/512, 0.0));
    vec4 yf = texture2D(nmap, coord.xy+vec2(0.0, 1.0/512));
    vec4 yb = texture2D(nmap, coord.xy-vec2(0.0, 1.0/512));
